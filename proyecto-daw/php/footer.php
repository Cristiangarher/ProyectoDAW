<footer style="background-color: black; color: white; padding: 40px 20px 10px 20px; font-family: Arial, sans-serif;">
    <div class="footer-container" style="display: flex; justify-content: space-between; align-items: flex-start; max-width: 1200px; margin: 0 auto; flex-wrap: wrap;">
        
        <div class="footer-column" style="flex: 1; min-width: 200px; margin-bottom: 20px;">
            <h2 style="font-family: 'Comic Sans MS', cursive; margin: 0; font-size: 28px; line-height: 1.1;">
                Ferretería<br>García
            </h2>
        </div>

        <div class="footer-column" style="flex: 1; min-width: 200px; margin-bottom: 20px;">
            <h3 style="font-size: 16px; margin-bottom: 15px;">Sobre Ferretería García</h3>
            <ul style="list-style: none; padding: 0; font-size: 14px; line-height: 2;">
                <li><a href="nosotros.php" style="color: white; text-decoration: none;">Quiénes somos</a></li>
                <li><a href="condiciones.php" style="color: white; text-decoration: none;">Condiciones generales</a></li>
                <li><a href="cookies.php" style="color: white; text-decoration: none;">Políticas de cookies</a></li>
            </ul>
        </div>

        <div class="footer-column" style="flex: 1; min-width: 200px; margin-bottom: 20px;">
            <h3 style="font-size: 16px; margin-bottom: 15px;">Compra con confianza</h3>
            <ul style="list-style: none; padding: 0; font-size: 14px; line-height: 2;">
                <li><a href="garantia.php" style="color: white; text-decoration: none;">Garantía compra segura</a></li>
                <li><a href="devoluciones.php" style="color: white; text-decoration: none;">Devoluciones y reembolso</a></li>
            </ul>
        </div>

        <div class="footer-column" style="flex: 1; min-width: 200px; margin-bottom: 20px; position: relative;">
            <div class="social-icons" style="text-align: right; margin-bottom: 15px;">
                <a href="#" style="color: white; margin-left: 15px; font-size: 20px;"><i class="fa-brands fa-instagram"></i></a>
                <a href="#" style="color: white; margin-left: 15px; font-size: 20px;"><i class="fa-brands fa-youtube"></i></a>
            </div>
            
            <h3 style="font-size: 16px; margin-bottom: 15px;">Atención al cliente</h3>
            <ul style="list-style: none; padding: 0; font-size: 14px; line-height: 2;">
                <li><a href="contacto.php" style="color: white; text-decoration: none;">Contáctanos</a></li>
                <li><a href="ayuda.php" style="color: white; text-decoration: none;">Ayuda</a></li>
            </ul>
        </div>
    </div>

    <div class="footer-bottom" style="border-top: 1px solid #444; margin-top: 30px; padding-top: 15px; text-align: center;">
        <p style="font-size: 14px; margin: 0; text-decoration: underline; color: #4A90E2;">
            @Copyright Ferretería García 2025. Todos los derechos reservados.
        </p>
    </div>
</footer>